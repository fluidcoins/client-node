...
describe("Fluidcoins", () => {
  ...
  describe("#Transctions", () => {
    ...
    it("Should Fetch a single transaction", async () => {
      try {
        const data = await fluidcoins.getSingleTransaction(
          transactions[0].reference
        );
        expect(data).to.be.an("object");
        expect(data.status).to.be.equal(true);
        expect(data.transaction).to.be.an("object");
        expect(data.transaction.reference).to.be.equal(
          transactions[0].reference
        );
      } catch (e) {
        expect(e.status).to.be.equal(false);
  ...
  describe("#Address", () => {
    ...
    it("Should new address with error message", async () => {
      try {
        ...
        expect(e.message).to.be.equal(
          "cannot generate USD Tether address in test mode."
        );
      }
    });
    ...
    it("Should create a new ETH address", async () => {
      try {
        const data = await fluidcoins.createNewAddress({
          code: "ETH",
          network: "ERC20",
        });
    ...
    it("Should Fetch a single transaction that occurred on a given address", async () => {
      try {
        const data = await fluidcoins.getAddressSingleTransaction(
          address.reference,
          transactions[0].reference
        );
        expect(data).to.be.an("object");
  ...
  describe("#Customer", () => {
    ...
    it("Should edit a single customer", async () => {
      try {
        const data = await fluidcoins.editCustomer(
          customer.reference,
          updateUserData
        );
        expect(data).to.be.an("object");
    ...
    it("Should Fetch a List of transactions belong to a specific customer", async () => {
      try {
        const data = await fluidcoins.getCustomerTransactions(
          customer.reference
        );
        expect(data).to.be.an("object");
        expect(data.status).to.be.equal(true);
  ...
  describe("#Payment", () => {
    ...
    it("Should Fetch all transactions for payement link", async () => {
      try {
        const data = await fluidcoins.getPaymentLinkTransactions(
          link.identifier
        );
        expect(data).to.be.an("object");
        expect(data.status).to.be.equal(true);
